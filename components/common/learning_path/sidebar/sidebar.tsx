...
const LPSidebar = ({ LP, editMode, toggleEdit, triggerSave }: { LP: Path, editMode: boolean, toggleEdit: React.Dispatch<React.SetStateAction<boolean>>, triggerSave: () => void }) => {
	return (
		<aside className="flex-1 w-1/3 flex flex-col gap-3 border border-r-0 border-gray-300 relative h-screen overflow-y-scroll bg-white transition-all p-3">
			{
				editMode ? <Button className="w-fit mx-auto" onClick={() => {
					triggerSave()
