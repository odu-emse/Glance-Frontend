...
interface EditableFieldProps {
	type: 'text' | 'area'
	inputDetails: {
		type: 'text' | 'email' | 'password' | 'url' | 'number' | 'search' | 'tel' | 'file',
		placeholder: string,
		defaultValue: string,
		label?: string,
		name: string,
...
function EditableField({type, isEditing, inputDetails} : EditableFieldProps ) {
	if(type === 'text') {
		...
		return (
			<h4 className='my-1 uppercase'>{inputDetails.defaultValue}</h4>
		)
	} else {
		if(isEditing) {
			return (
		...
		return (
			<p className='my-1 uppercase'>{inputDetails.defaultValue}</p>
		)
	}
}
