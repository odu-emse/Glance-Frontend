...
export const ModuleItem = ({ data, role, expanded, handleExpansion, selected }: ModuleItemProps) => {
	...
	return (
			<div className={`px-5 py-4 rounded-md shadow cursor-pointer flex justify-center items-center w-full border-2 ${selected ? "border-royalblue" : "border-transparent"}`} >
				<div className="flex grow flex-col gap-1" onClick={() => handleExpansion(!expanded)}>
					<div
						className="flex flex-row gap-1 sans text-sm sans uppercase items-center"
					>
						<p className="p-0 m-0">
					...
					<div>
						<h2 className="sans leading-5 text-royalblue font-semibold">{data.name}</h2>
					</div>
				</div>
