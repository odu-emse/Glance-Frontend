...
const GlobalUserContext = createContext({
	...
	user: Omit<UserAccount & { openID: string, biography?: string }, 'id'> & {id: string} | null
	setUser: (user: Omit<UserAccount & { openID: string, biography?: string }, 'id'> & {id: string} | null) => void
})
