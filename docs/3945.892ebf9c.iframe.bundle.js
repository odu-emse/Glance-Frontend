	...
	[3945],
	{
				...
				if (file && acceptedFiles) {
					...
					return acceptedFilesArray.some(function (type) {
						...
						return '.' === validType.charAt(0)
							? fileName.toLowerCase().endsWith(validType)
							: validType.endsWith('/*')
		...
		'./node_modules/react-dropzone/dist/es/index.js': (
			...
			function toFileWithPath(file, path) {
				var f = (function withMimeType(file) {
					var name = file.name
					if (name && -1 !== name.lastIndexOf('.') && !file.type) {
						var ext = name.split('.').pop().toLowerCase(),
							type = COMMON_MIME_TYPES.get(ext)
						type &&
				...
				if ('string' != typeof f.path) {
					var webkitRelativePath = file.webkitRelativePath
					Object.defineProperty(f, 'path', {
						value:
							'string' == typeof path
								? path
								: 'string' == typeof webkitRelativePath &&
						...
						switch (_a.label) {
							case 0:
								return dt.items
									...
									: [3, 2]
							case 1:
								return [2, noIgnoredFiles(flatten(_a.sent()))]
							case 2:
								return [
									2,
									noIgnoredFiles(
			...
			function noIgnoredFiles(files) {
				return files.filter(function (file) {
					return -1 === FILES_TO_IGNORE.indexOf(file.name)
				})
			}
			function fromList(items) {
				if (null === items) return []
				for (var files = [], i = 0; i < items.length; i++) {
					var file = items[i]
					files.push(file)
				}
				return files
			...
			var getInvalidTypeRejectionErr =
					function getInvalidTypeRejectionErr(accept) {
						accept =
							Array.isArray(accept) && 1 === accept.length
								? accept[0]
								: accept
			...
			function fileAccepted(file, accept) {
				var isAcceptable =
					'application/x-moz-file' === file.type ||
					(0, es.Z)(file, accept)
				return [
					isAcceptable,
				...
				if (isDefined(file.size))
					if (isDefined(minSize) && isDefined(maxSize)) {
						if (file.size > maxSize)
							return [!1, getTooLargeRejectionErr(maxSize)]
						if (file.size < minSize)
							return [!1, getTooSmallRejectionErr(minSize)]
					} else {
			...
			function isPropagationStopped(event) {
				return 'function' == typeof event.isPropagationStopped
					? event.isPropagationStopped()
					: void 0 !== event.cancelBubble && event.cancelBubble
			}
			...
			function es_unsupportedIterableToArray(o, minLen) {
				if (o) {
					if ('string' == typeof o)
						return es_arrayLikeToArray(o, minLen)
					var n = Object.prototype.toString.call(o).slice(8, -1)
					return (
						'Object' === n &&
							o.constructor &&
			...
			var defaultProps = {
				disabled: !1,
				getFilesFromEvent: function fromEvent(evt) {
					return __awaiter(this, void 0, void 0, function () {
						return __generator(this, function (_a) {
							return isObject(evt) &&
								...
								: Array.isArray(evt) &&
								  evt.every(function (item) {
										return (
											'getFile' in item &&
											'function' == typeof item.getFile
										)
								  })
								? [2, getFsHandleFiles(evt)]
								: [2, []]
						})
					})
				},
			...
			function useDropzone() {
				var props =
					...
					preventDropOnDocument =
						_defaultProps$props.preventDropOnDocument,
					noClick = _defaultProps$props.noClick,
					noKeyboard = _defaultProps$props.noKeyboard,
					noDrag = _defaultProps$props.noDrag,
					noDragEventsBubbling =
						_defaultProps$props.noDragEventsBubbling,
					onError = _defaultProps$props.onError,
					validator = _defaultProps$props.validator,
					acceptAttr = (0, react.useMemo)(
						function () {
							return (function acceptPropAsAcceptAttr(accept) {
								if (isDefined(accept))
					...
					pickerTypes = (0, react.useMemo)(
						function () {
							return (function pickerOptionsFromAccept(accept) {
								return isDefined(accept)
									? [
											{
												description: 'Files',
					...
					onFileDialogOpenCb = (0, react.useMemo)(
						function () {
							return 'function' == typeof onFileDialogOpen
								? onFileDialogOpen
								: noop
						},
						[onFileDialogOpen],
					...
					onFileDialogCancelCb = (0, react.useMemo)(
						function () {
							return 'function' == typeof onFileDialogCancel
								? onFileDialogCancel
								: noop
						},
						[onFileDialogCancel],
					...
					fsAccessApiWorksRef = (0, react.useRef)(
				...
				var onErrCb = (0, react.useCallback)(
						...
						function () {
							if (fsAccessApiWorksRef.current) {
								dispatch({ type: 'openDialog' }),
									onFileDialogOpenCb()
								var opts = { multiple, types: pickerTypes }
								window
					...
					onBlurCb = (0, react.useCallback)(function () {
						dispatch({ type: 'blur' })
					}, []),
					onClickCb = (0, react.useCallback)(
						function () {
							noClick ||
								(!(function isIeOrEdge() {
									var userAgent =
					...
					composeDragHandler = function composeDragHandler(fn) {
						return noDrag ? null : composeHandler(fn)
					},
					stopPropagation = function stopPropagation(event) {
						noDragEventsBubbling && event.stopPropagation()
					},
					getRootProps = (0, react.useMemo)(
						function () {
							return function () {
								var _ref2 =
										arguments.length > 0 &&
