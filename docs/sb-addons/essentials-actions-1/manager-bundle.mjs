try {
	...
	var Yn = __STORYBOOKCOREEVENTS__,
		{
			...
			FORCE_RE_RENDER: Qn,
			GLOBALS_UPDATED: ea,
			IGNORED_EXCEPTION: ta,
			NAVIGATE_URL: ra,
			PLAY_FUNCTION_THREW_EXCEPTION: na,
			PRELOAD_ENTRIES: aa,
			PREVIEW_BUILDER_PROGRESS: oa,
			PREVIEW_KEYDOWN: ia,
			...
			SET_INDEX: da,
			SET_STORIES: ma,
			SHARED_STATE_CHANGED: ba,
			SHARED_STATE_SET: ga,
			STORIES_COLLAPSE_ALL: ha,
			STORIES_EXPAND_ALL: Ea,
			STORY_ARGS_UPDATED: ya,
			STORY_CHANGED: I,
			STORY_ERRORED: Oa,
			...
			STORY_PREPARED: _a,
			STORY_RENDERED: Ra,
			STORY_RENDER_PHASE_CHANGED: Sa,
			STORY_SPECIFIED: Aa,
			STORY_THREW_EXCEPTION: Ca,
			STORY_UNCHANGED: xa,
			UPDATE_GLOBALS: wa,
			UPDATE_QUERY_PARAMS: Na,
	...
	var Lt = ({ rows: e, columns: t, rowsData: r }) => {
		...
		jt = ({ isCloseTag: e, data: t, expanded: r }) => {
			...
			switch (t.nodeType) {
				...
				case Node.DOCUMENT_FRAGMENT_NODE:
					return a.createElement('span', null, t.nodeName)
				default:
					return a.createElement('span', null, Ht[t.nodeType])
			}
		},
		Ut = function* (e) {
			if (e && e.childNodes) {
		...
		kt = (e) =>
			a.createElement(We, { nodeRenderer: jt, dataIterator: Ut, ...e }),
		Gt = ue(kt),
		Vt = dt(gt()),
	...
	var Po = __STORYBOOKCOMPONENTS__,
		{
			A: Bo,
			ActionBar: Xe,
			AddonPanel: Fo,
			...
			codeCommon: Bi,
			components: Fi,
			createCopyToClipboardFunction: zi,
			getStoryHref: Hi,
			icons: ji,
			interleaveSeparators: Ui,
			nameSpaceClassNames: ki,
			resetComponents: Gi,
			withReset: Vi,
	...
	function $(e) {
		...
		return (
			($ = function (n) {
				...
				function o() {
					return P(n, arguments, V(this).constructor)
				}
				return (
	...
	function hr(e, t) {
		...
		var r = B(t),
			...
			o = v({}, r, {
				alpha: F(0, 1, (n * 100 + parseFloat(e) * 100) / 100),
			})
		return te(o)
	}
	var Er = R(hr),
	...
	var Ar = M.div({
		...
		xr = M.div({ flex: 1, padding: '0 0 0 5px' }),
		...
		Mr = class extends we {
			...
			componentDidMount() {
				...
				e.on(U, this.addAction), e.on(I, this.handleStoryChange)
			}
			componentWillUnmount() {
				this.mounted = !1
				let { api: e } = this.props
				e.off(I, this.handleStoryChange), e.off(U, this.addAction)
			}
			render() {
