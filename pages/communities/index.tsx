		...
		<div className='mx-8 flex'>
			<div className='m-10 grow'>
				...
				<div className='flex items-center justify-between'>
					...
					<div className='flex flex-row gap-1'>
						...
						<div className='flex gap-2 w-fit relative'>
							...
							>FILTER</Button>
						{isSorted && <div className='flex flex-col absolute bg-white top-11 left-0 border border-royalblue rounded-sm px-2 py-1 w-fit'>
							<ul className='list-none ml-0 my-0 sans uppercase text-sm'>
								<li className="hover:underline cursor-pointer text-royalblue"
									onClick={() => setSortType(
										'newest'
									)}
								>
							...
							{isFiltered && <div className='flex flex-col absolute bg-white top-11 right-0 border border-royalblue rounded-sm px-2 py-1 w-fit'>
								<div className='text-sm'>
									<h4 className="font-medium uppercase">Topics</h4>
								<ul className='list-none ml-0 my-0'>
									<li className="hover:underline cursor-pointer text-royalblue uppercase sans"
									    onClick={() => {
												setFilterType(
													{
														label: 'all',
				...
				<div className='m-2'>
					{data.threads
						.filter((v) => {
							if (filterType.label === 'all' && searchQuery === '') {
								return v.title !== null
							}
							if (filterType.label === 'topic' && searchQuery === '') {
						...
						.sort(
							(a, b) => {
								if(sortType === 'newest')
									return new Date(b.updatedAt).valueOf() -
									new Date(a.updatedAt).valueOf()
								else if(sortType === 'oldest')
									return new Date(a.updatedAt).valueOf() -
									new Date(b.updatedAt).valueOf()
								else if(sortType === 'mostWatched')
									return b.usersWatching?.length || 0 - a.usersWatching?.length || 0
