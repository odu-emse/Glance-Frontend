...
const Index = ({ }) => {
	...
	}, gqlFetcher) as {data: User[], error: Error}
		...
		<div className="m-8 flex">
			<div className="m-10 grow">
				<div className="flex my-2 items-center">
					<img
						src={session?.user.image || "https://i.pinimg.com/custom_covers/222x/85498161615209203_1636332751.jpg"}
						alt="profile image"
						className="shadow-lg rounded-full max-w-full h-4 align-middle border-none"
					/>
					<small className="pl-2 font-bold">{session?.user.name}</small>
					<small>
				...
				<div className="m-2">
					{data.moduleEnrollment.filter(v => v.status !== 'INACTIVE').map(enrollment => {
						return enrollment.module.collections.map(collection => {
							return collection.lessons.map(lesson => {
								return lesson.threads.map((thread, threadMapIndex) => {
									console.log(thread);
									return (
