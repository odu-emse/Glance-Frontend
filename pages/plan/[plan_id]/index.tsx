					...
					<ul className="m-0 list-none divide-y">
						{
							data.learningPath[0].paths.map((path, index) => {
								return (
									<li key={index} className="flex justify-between items-center gap-1 my-1 sans">
										<p className="">{path.course.name}</p>
										<span className="text-sm py-1 rounded-md bg-wgray font-semibold px-2">{path.status}</span>
										<time className="">{path.hoursSatisfies} hours</time>
										<Link href={`/plan/${session.openId}/paths/${path.id}`}>
...
const PlanOfStudyPanel = ({ title, children } : { title: string, children?: React.ReactNode[] | React.ReactNode} ) => {
	return (
		<div className="col-span-1 row-span-1 h-64 border border-wgray rounded-sm p-3">
			<h3>{title}</h3>
			<>
				{
					Array.isArray(children) ? children.map((child, index) => <div key={index}>{child}</div>) : children
				}
			</>
		</div>
	)
