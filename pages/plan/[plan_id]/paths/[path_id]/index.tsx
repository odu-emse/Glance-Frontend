				...
				<div className='flex-none w-2/3'>
					<ul className='list-none m-0'>
						{LP.course.sections.map((section, index) =>
							<SectionCard section={section} key={index} collapseIndex={collapseSection} sectionIndex={index} handleCollapse={setCollapseSection} sectionPointer={sectionRef} />,
						)}
					</ul>
				</div>
				<LPSidebar LP={LP} />
...
const SectionCard = ({ section, sectionIndex, collapseIndex, handleCollapse, sectionPointer }: {
	section: SectionPath,
	sectionIndex: number,
	collapseIndex: number,
	handleCollapse: React.Dispatch<
	...
	return (
		<li>
			...
			<ul className='list-none divide-y'>
				{collection.modules.map((module, index) => <ModuleCard key={index} module={module} />)}
			</ul>
		</li>
	);
};
...
const ModuleCard = ({ module }: {
	...
	return (
		...
		>
			...
			<ul className="mt-0">
				{module.objectives.map((item, index) => <li className="sans" key={index}>{item}</li>)}
			</ul>
			<ModuleTagList title={'Topics covered'} listData={module.keywords} elementName={null} elementLink={null} />
			<ModuleTagList title={'Recommended to complete first'} listData={module.parentModules} elementName={'moduleName'} elementLink={'id'} />
			<ModuleTagList title={'Possible next steps...'} listData={module.subModules} elementName={'moduleName'} elementLink={'id'} />
		</li>
	);
};

const ModuleTagList = ({ title, listData, elementName, elementLink }: {
	title: string,
			...
			<div className='flex gap-1'>
				{
					listData.map((item, index) => {
						if(typeof item === 'string'){
							return (
								<span className='flex gap-2 bg-wgray rounded px-2 py-1 sans text-sm w-fit' key={index}>
									<h6>
										{item}
									</h6>
								</span>
						...
						return (
							<Link href={`/modules/${item[elementLink]}`} key={index}>
