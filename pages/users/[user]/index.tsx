...
const UserProfile = () => {
	...
	const [updatedProfile, setUpdatedProfile] = useState<UserProfileType | null>(null)
	...
	const updateSocial = (accountID:string, input: {
		github: string | null
		linkedin: string | null
		portfolio: string | null
		facebook: string | null
			...
			<div className='flex flex-col md:flex-row mt-3'>
				<aside className='flex-none flex-col mr-8 mx-6'>
					<div className='relative h-48 w-48 mb-5 rounded-full overflow-clip mx-auto group'>
						<div
							className='opacity-0 group-hover:opacity-100 w-full h-full z-10 absolute left-0 transition-opacity top-0 uppercase font-black text-white text-xl tracking-wider flex justify-center items-center'>
							<span className='cursor-pointer'>Edit</span>
						</div>
						<img src={user.picURL} alt='The profile picture of the current user' />
								...
								(
									...
									<Button
										onClick={() => {
											...
											updateSocial(account.id, updatedProfile.social);
				...
				<div className='mx-10'>
					<div className='mb-3'>
						<h4 className='m-0 uppercase font-bold'>Contact Information:</h4>
						{!isEditMode ? (
							<>
					...
					<div className='SocialMedia mb-3'>
						...
						{!isEditMode ? (
							<>
								<h4 className='my-1 font-medium'>Github: {user.social?.github}</h4>
								<h4 className='my-1 font-medium'>Twitter: {user.social?.twitter}</h4>
								<h4 className='my-1 font-medium'>LinkedIn: {user.social?.linkedin}</h4>
								<h4 className='my-1 font-medium'>Facebook: {user.social?.facebook}</h4>
								<h4 className='my-1 font-medium'>Portfolio: {user.social?.portfolio}</h4>
							</>
						) : (
							<>
								<Input
									type='url'
									placeholder='GITHUB:'
									defaultValue={user.social?.github} label={null} name={'github'} onChange={(e:string) => {
									setUpdatedProfile((prevState) => ({
										...prevState,
										social: { ...prevState?.social, github: e },
									}))
								}} />
								<Input
									type='text'
									...
									onChange={(e:string) => {
										setUpdatedProfile((prevState) => ({
											...prevState,
											social: { ...prevState?.social, twitter: e },
										}))
									}}
									 />
								<Input
									...
									defaultValue={user.social?.linkedin} label={null} name={'linkedin'} onChange={(e:string) => {
									setUpdatedProfile((prevState) => ({
										...prevState,
										social: { ...prevState?.social, linkedin: e },
									}))
								}}  />
								<Input
									type='text'
									placeholder='FACEBOOK:'
									defaultValue={user.social?.facebook} label={null} name={'facebook'} onChange={(e:string) => {
									setUpdatedProfile((prevState) => ({
										...prevState,
										social: { ...prevState?.social, facebook: e },
									}))
								}} options={[]} />
								<Input
									type='url'
									placeholder='PORTFOLIO:'
									defaultValue={user.social?.portfolio} label={null} name={'portfolio'} onChange={(e:string) => {
									setUpdatedProfile((prevState) => ({
										...prevState,
										social: { ...prevState?.social, portfolio: e },
									}))
								}} options={[]} />
							</>
						)}
