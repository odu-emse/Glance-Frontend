...
export const InformationCard = ({
   ...
   return(
       <div className="border-solid border-gray rounded border shadow-lg flex justify-between w-2/5">
        {
        percentComplete && (
                <>
        ...
        <div className="checkIcon">
            <FaRegCheckCircle fill="green" size="30px"/>
        </div>
        <div className="modueleInformation px-2">
            <p className="mx-2 my-2 text-l">
                {statusMessage}
            </p>
       ...
       <div className="closeIcons flex gap-2 justify-items-end my-10 mx-2">
            <BsFillExclamationOctagonFill size="15px" fill="gray" onClick={handle}/>
            <MdClose fill="gray" size="20px" onClick={handle}/>
       </div>
    </>
    )}
    ...
    testType && (
        ...
        <BsFillBellFill size="20px" onClick={handle}/>
        </div>
        </>
    )
    }
        ...
        grade && (
            <>
                <div className='m-3 flex flex-col'>
                   <div className="module text-lg font-medium"> 
                   {moduleIdentifier}  </div>
                   <div className="flex justify-start items-center text-gray-600 inline-block text-sm">
                    {percentage}% passing rate </div>
                   <div className="flex gap-1 items-center text-gray-500 inline-block text-xs"> 
                   Average Grade: {grade} </div>
                </div>
                <div className="moreOptions grid justify-items-end m-2 text-gray-500 inline-block text-sm"
                onClick={handle}
                >
                more
                </div>
